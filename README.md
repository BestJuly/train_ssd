...
Here is an instruction in English. A Chinese version is here:[ﾖﾐﾎﾄｰ豎ｾ](https://github.com/BestJuly/train_ssd/blob/master/readme_zh.md).
...
mkdir mydataset
```
**2.** copy `create_list.sh` 縲～create_data.sh`縲～labelmap_voc.prototxt` under `/data/VOC0712` to your own folder `/mydataset`
```
...
-- mydataset
       | -- VOC2007
               ...
               | -- ImageSets   
                        | -- Layout         # trainval.txt縲》rain.txt縲」al.txt縲》est.txt
                        | -- Main           # trainval.txt縲》rain.txt縲」al.txt縲》est.txt
                        | -- Segmentation   # trainval.txt縲》rain.txt縲」al.txt縲》est.txt
               | -- JPEGImages  # your image files
```
**5.** modify `create_list.sh`縲～create_data.sh` according to your own condition and run in sequence to get lmdb files and other necessary files   

**6.** modify `/examples/ssd/ssd_pascal.py`
```
train_data      # lmdb file path for training 
test_data       # lmdb file path for testing
save_dir縲《napshot_dir縲）ob_dir縲｛utput_result_dir
name_size_file縲〕abel_map_file
